...
namespace GameLib
{
	...
	void sprite_render2(Sprite* pSpr,
		float x, float y, float sx, float sy,
		float tx, float ty, float tw, float th,
		float cx, float cy, float angle,
		float r, float g, float b, float a,
		bool world)
	{
		if (pSpr)
		{
			if (world)
			{
				...
				VECTOR2 s = VECTOR2(sx, sy) * scaleFactor;
				VECTOR2 pos = view::worldToScreen(VECTOR2(x, y));
				pSpr->render2(m.context, pos.x, pos.y, s.x, s.y, tx, ty, tw, th, cx, cy, angle, r, g, b, a);
			}
			else
			{
				pSpr->render2(m.context, x, y, sx, sy, tx, ty, tw, th, cx, cy, angle, r, g, b, a);
			}
		}
	}

    void sprite_render3(Sprite* pSpr,
        float x, float y, float sx, float sy,
        float tx, float ty, float tw, float th,
        float cx, float cy, float angle,
        float r, float g, float b, float a,
        ...
        bool world)
    {
        if (pSpr)
        {
            if (world)
            {
                ...
                VECTOR2 s = VECTOR2(sx, sy) * scaleFactor;
                VECTOR2 pos = view::worldToScreen(VECTOR2(x, y));
                pSpr->render3(m.context, pos.x, pos.y, s.x, s.y, tx, ty, tw, th, cx, cy, angle, r, g, b, a, ratio);
            }
            else
            {
                pSpr->render3(m.context, x, y, sx, sy, tx, ty, tw, th, cx, cy, angle, r, g, b, a, ratio);
            }
        }
    }
