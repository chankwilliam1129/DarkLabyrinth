...
namespace GameLib
{
    ...
    Sprite::ConstantBuffer Sprite::cb = {};
    int Sprite::pixelType = Sprite::PixelType::dNormal;
    //==========================================================================
    //
    //      Spriteクラス
    //
        ...
        rasterizerState(nullptr), shaderResourceView(nullptr), samplerState(nullptr)
    {
        ...
        if (!ResourceManager::loadPixelShader(device, "./Data/Shaders/sprite_ps_light.cso", &pixelShader_light))
        {
            assert(!"ライトピクセルシェーダーの作成に失敗(Sprite)");
            return;
        }

        if (!ResourceManager::loadPixelShader(device, "./Data/Shaders/sprite_ps_opacity.cso", &pixelShader_Opacity))
        {
            assert(!"ライトピクセルシェーダーの作成に失敗(Sprite)");
            return;
        }
        ...
        const VECTOR4& color) const
    {
        ...
        switch (pixelType) 
        {
        case dNormal:context->PSSetShader(pixelShader, nullptr, 0);break;
        case dLight:context->PSSetShader(pixelShader_light, nullptr, 0);break;
        case dOpacity:context->PSSetShader(pixelShader_Opacity, nullptr, 0); break;
        }
    ...
    void Sprite::render2(ID3D11DeviceContext* context,
        const VECTOR2& position, const VECTOR2& scale,
        const VECTOR2& texPos, const VECTOR2& texSize,
        const VECTOR2& center, float angle,
        const VECTOR4& color) const
    {
        ...
        for (int i = 0; i < 4; i++)
        {
			...
			vertices[i].position.x *= (tw);
			vertices[i].position.y *= (th);
    ...
    void Sprite::render3(ID3D11DeviceContext* context,
        const VECTOR2& position, const VECTOR2& scale,
        const VECTOR2& texPos, const VECTOR2& texSize,
        const VECTOR2& center, float angle,
        const VECTOR4& color, float ratio) const
    {
        if (scale.x == 0.0f || scale.y == 0.0f) return;
        ...
        float sinValue = sinf(angle);
        float cosValue = cosf(angle);
        VECTOR3 posValue = VECTOR3(cosValue, sinValue * ratio, 0.0f) * 0.5f;

        vertex vertices[4] = {};

        vertices[0] = { VECTOR3(0.5f, 1.0f, 0) + posValue, color, VECTOR2(0, 1) };
        vertices[1] = { VECTOR3(0.5f, 1.0f, 0) - posValue, color, VECTOR2(1, 1) };
        vertices[2] = { VECTOR3(0.5f, 0.0f, 0) + posValue, color, VECTOR2(0, 0) };
        vertices[3] = { VECTOR3(0.5f, 0.0f, 0) - posValue, color, VECTOR2(1, 0) };
    ...
    {
        return render2(context, VECTOR2(x, y), VECTOR2(sx, sy), VECTOR2(tx, ty), VECTOR2(tw, th), VECTOR2(cx, cy), angle, VECTOR4(r, g, b, a));
    }
    ...
    {
        return render3(context, VECTOR2(x, y), VECTOR2(sx, sy), VECTOR2(tx, ty), VECTOR2(tw, th), VECTOR2(cx, cy), angle, VECTOR4(r, g, b, a), ratio);
    }
